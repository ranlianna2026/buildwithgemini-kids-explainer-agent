    ...
    for i, s in enumerate(scenes):
        ...
        if has_tts:
            try:
                ...
                # Transform TTS narration tone directly to Parent's voice pitch tone (no concatenation)
                if has_parent_voice:
                    # Parent voice profile: +18% pitch shift with tempo correction
                    pitch_filter = "asetrate=24000*1.18,aresample=24000,atempo=0.847"
                else:
                    # Warm default pitch shift with tempo correction
                    pitch_filter = "asetrate=24000*1.10,aresample=24000,atempo=0.909"
